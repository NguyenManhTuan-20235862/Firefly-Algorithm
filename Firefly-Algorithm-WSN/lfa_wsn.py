...
from lfa_move import lfa_move
from coverage import coverage
from findlimits import findlimits

def lfa_wsn(u0, Lb, Ub, para, q):
    n = int(para[0])
    ...
    for iter in range(MaxGeneration):
        print(f'LFA Iteration iter = {iter+1}')
        ...
        print(f'LFA coverage of current solution: {maxzn[-1]}')
        
        nsxo = nsx.copy()
        nsyo = nsy.copy()
        Lighto = Lightn.copy()
        
        nsx, nsy = lfa_move(n, nsx, nsy, Lightn, nsxo, nsyo, Lighto, alpha, betamin, gamma, b, levy_beta)
